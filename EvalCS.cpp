...
//compilación: g++ -std=c++17 -O3 EvalCS.cpp -o evCS
//ejecución: ./evCS
...
std::vector<GTEntry> read_ground_truth(const std::string& filename) {
    ...
    while (std::getline(in, line)) {
        if (header) { header = false; continue; } // saltar la cabecera
        ...
        GTEntry e;
        e.kmer = cols[2]; // columna kmer
        e.count = std::stoll(cols[3]); // columna count
        data.push_back(e);
    }
    return data;
}
...
int main() {
    //tamaño del count sketch y valor de k
    const int d = 32;
    const int w = 10'000'000;
    const size_t K = 31;
    ...
    std::cout << "[STATS] Memoria (MB):      " << cs.size_MB() << "\n";
